...
public:
    dtNoPivLU();
    dtNoPivLU(const m_type *element, const size_t n_byte);
    dtNoPivLU(const dtMatrix<m_row, m_col, m_type> &m);
    dtNoPivLU(const dtMatrix3<m_type, m_row, m_col> &m);
    ...
    dtMatrix<m_row, m_col, m_type> GetMatrix() const;  // return matrix A including L and U matrix
